...
- `lua/sparrow/init.lua` exposes the public API that users bind to keymaps (via `require('sparrow')`).
...
- `lua/sparrow/util_treesitter.lua` is the shared Tree-sitter utility layer:
...
- Query names are the filenames (without `.scm`), e.g. `queries/go/nav_func_decl.scm` is queried as `nav_func_decl`.
...
- `lua/sparrow/nav_func_decl.lua`
- Uses the named query `nav_func_decl` and moves to the next/previous `@func_decl_start`.
...
- `lua/sparrow/nav_func_equal.lua`
...
- `lua/sparrow/nav_identifier.lua`
...
- `lua/sparrow/range_action.lua`
- Finds the nearest enclosing function-like node at the cursor using a language-specific query:
  - default: `nav_func_decl`
  - lua: `nav_range_action` (has richer patterns and `@func_node` captures)
- Exposes actions:
...
- For function-call/expression navigation, add `<newlang>` to `QUERY_NAMES` in `lua/sparrow/nav_func_equal.lua`.
- For identifier navigation, add a `<newlang>` entry to `QUERY_STRINGS` in `lua/sparrow/nav_identifier.lua` (it does not use `queries/<lang>/` today).
